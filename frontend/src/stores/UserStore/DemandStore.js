...
export const useDemandStore = defineStore('demandsStore', {
  ...
  actions: {
    ...
    async fetchMotifs() {
      try {
        const response = await axios.get('/motifs')
    ...
    async createMotif(title) {
      this.showLoader = true
      try {
        const response = await axios.post('/motifs', { title })
    ...
    async updateMotif(id, title) {
      this.showLoader = true
      try {
        await axios.patch(`/motifs/${id}`, { title })
    ...
    async archiveMotif(id) {
      this.showLoader = true
      try {
        await axios.patch(`/motifs/${id}/archive`)
    ...
    async unarchiveMotif(id) {
      this.showLoader = true;
      try {
        const response = await axios.patch(`/motifs/${id}/unarchive`);
    ...
    async updateRequest(updatedRequest) {
      ...
      try {
        const response = await axios.patch(
          `/requests/${updatedRequest.id}/update`,
          updatedRequest,
          {
            headers: {
              Authorization: `Bearer ${token}`,
